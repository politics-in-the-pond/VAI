...
def compare(x, y):
    x = int(x.split(f'\\')[-1].split(r'_')[0])
    y = int(y.split(f'\\')[-1].split(r'_')[0])
    if x > y:
        return 1
    elif x == y:
        return 0
...
class AudioDataLoader(torch.utils.data.Dataset):
    def __init__(self, path, hparams=None):
        ...
        self.audiopaths.sort(key=cmp_to_key(compare))
    ...
    def get_data(self, audiopath):
        ...
        assert sr == self.hparams.sampling_rate, "sampling rate doesn't match with configs"
        wav_norm = torch.FloatTensor(wav) #/ self.hparams.max_wav_value
